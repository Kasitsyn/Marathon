...
function saveToCookie(item, value, ) {
    setCookie('favoriteCity', 'Moscow', { secure: true, 'max-age': 3600 });
}
...
function renderNow() {
    if (Object.keys(storage).length === 0) {
        ...
        UI.INPUT_CITY.value = getCookie('cityName') ? getCookie('cityName') : ''
    }
}
...
function getData(e) {
    ...
    setCookie('cityName', cityName, { secure: true, 'max-age': 3600 })
...
function convertDate(params) {
    ...
    const convertedDate = format(date, 'HH:mm:ss')
