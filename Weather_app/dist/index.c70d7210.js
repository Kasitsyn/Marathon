...
})({"9Yzim":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "139570cfc70d7210";
...
},{}],"4Xv8N":[function(require,module,exports) {
...
onLoad();
function onLoad() {
    const NOW_TAB = _viewJs.UI.TABS_BTN[0];
    const DETAILS_TAB = _viewJs.UI.TABS_BTN[1];
    const FORCAST_TAB = _viewJs.UI.TABS_BTN[2];
    getDataFromLocalStorage();
    _viewJs.UI.TABS_BTN.forEach(makeCurrentTabActviated);
    _viewJs.UI.INPUT_CITY.addEventListener('keydown', (e)=>{
        if (e.key === 'Enter') getData(e).then(()=>{
            renderNow();
            renderDetails();
        });
    });
    _viewJs.UI.NOW_BOOKMARK.addEventListener('click', addToBookmarks);
    renderNow();
    renderDetails();
    renderBookmarks();
    renderForecast();
...
function renderNow() {
    if (Object.keys(storage).length === 0) _viewJs.UI.NOW_TEMP.textContent = '°';
    else {
        _viewJs.UI.NOW_TEMP.textContent = `${storage.temperature}°`;
        _viewJs.UI.NOW_CITY.textContent = storage.city;
        _viewJs.UI.NOW_IMG.src = storage.img;
        if (Array.from(bookmarks).includes(storage.city.toLowerCase())) _viewJs.UI.NOW_BOOKMARK.setAttribute('hidden', 'true');
        else _viewJs.UI.NOW_BOOKMARK.removeAttribute('hidden');
        _viewJs.UI.INPUT_CITY.value = '';
    }
}
function renderDetails() {
    _viewJs.UI.DETAILS.textContent = '';
    _viewJs.UI.DETAILS.insertAdjacentHTML('afterbegin', `<p>${storage.city}</p>
        <ul>
...
function renderForecast() {
    _viewJs.UI.FORECAST.textContent = '';
    if (storage.forecast.list !== '') {
        storage.forecast.list.forEach((elem)=>{
            ...
            _viewJs.UI.FORECAST.insertAdjacentHTML('afterbegin', `<div>
                <p>${elem.dt_txt}</p>
        ...
        _viewJs.UI.FORECAST.insertAdjacentHTML('afterbegin', `<p>${storage.forecast.city}</p>`);
    }
}
function renderBookmarks() {
    _viewJs.UI.LOCATIONS.textContent = '';
    bookmarks.forEach((elem)=>{
        _viewJs.UI.LOCATIONS.insertAdjacentHTML('afterbegin', `<li><button class="locations__btn">${elem}</button><button class="btn__close">X</button></li>`);
        _viewJs.UI.LOCATIONS_CITY = document.querySelector('.locations__btn');
        _viewJs.UI.LOCATIONS_CITY_DELETE = document.querySelector('.btn__close');
        _viewJs.UI.LOCATIONS_CITY_DELETE.addEventListener('click', deleteBookmark);
        _viewJs.UI.LOCATIONS_CITY.addEventListener('click', (e)=>{
            getData(e).then(()=>{
                renderNow();
                renderDetails();
                renderForecast();
...
function getData(e) {
    ...
    return fetch(weatherUrl).then((response)=>response.json()
        ...
        storage.img = `https://openweathermap.org/img/wn/${data.weather.find((elem)=>elem.icon
        ).icon}@2x.png`;
    ...
    }).then(()=>fetch(forecastURL).then((response)=>response.json()
        ).then((data)=>{
...
function addToBookmarks(e) {
    bookmarks.add(_viewJs.UI.NOW_CITY.textContent.toLowerCase());
    // bookmarks.push(UI.NOW_CITY.textContent.toLowerCase())
    _viewJs.UI.NOW_BOOKMARK.setAttribute('hidden', 'true');
...
function makeCurrentTabActviated(elem) {
    elem.addEventListener('click', (e)=>{
        _viewJs.UI.TABS_BTN.forEach((el)=>{
...
function saveToLocalStorage(name, storage1) {
    localStorage[name] = JSON.stringify(storage1);
}
...
},{"./view.js":"opfVj","../../node_modules/date-fns":"9yHCA"}],"opfVj":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "UI", ()=>UI
);
const UI = {
    TABS_BTN: document.querySelectorAll('.tabs__item'),
    TABS_CONTENT: document.querySelectorAll('.tabs__block'),
    ...
    FORECAST: document.querySelector('#tab_03')
};
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["9Yzim","4Xv8N"], "4Xv8N", "parcelRequire405c")

//# sourceMappingURL=index.c70d7210.js.map
